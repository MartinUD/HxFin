<script lang="ts">
	...
	let costDialogCategoryName = $derived(categoryMap.get(dialogCostCategoryId)?.name ?? '');

	function setCostDialogState(nextState: ReturnType<typeof createEmptyCostDialogState>): void {
...
<div class="budget-page">
	...
	<div class="budget-toolbar">
			...
			<div class="budget-toolbar-filters">
				<Tabs.Root bind:value={selectedCategoryFilter}>
					<Tabs.List class="budget-category-list">
						<Tabs.Trigger
							value="all"
							class="budget-category-chip"
						>
							All
							<span class="ml-1.5 opacity-60 text-[10px]">{costs.filter((c: RecurringCost) => c.isActive).length}</span>
						</Tabs.Trigger>
						{#each categories as cat}
							...
							>
								<span class="w-2 h-2 rounded-full flex-shrink-0" style="background: {cat.color ?? DEFAULT_COLOR}"></span>
								{cat.name}
							</Tabs.Trigger>
						{/each}
					</Tabs.List>
		...
		<div class="budget-actions">
			...
			<Button size="sm" onclick={openAddCostDialog} disabled={categories.length === 0}>
				+ Add Cost
			</Button>
		</div>
	</div>
...
<Dialog.Root bind:open={costDialogOpen}>
	<Dialog.Content class="bg-card border-border sm:max-w-[440px]">
		<Dialog.Header>
			<Dialog.Title class="text-foreground">{costDialogMode === 'add' ? 'Add Cost' : 'Edit Cost'}</Dialog.Title>
			<Dialog.Description class="text-muted-foreground text-sm">
				{costDialogMode === 'add' ? 'Add a new recurring expense.' : 'Update this recurring expense.'}
			</Dialog.Description>
		</Dialog.Header>
		...
		<div class="flex flex-col gap-4 py-2">
			...
			<!-- Period -->
			<div class="flex flex-col gap-1.5">
				<ToggleGroup label="Period" options={[...PERIOD_OPTIONS]} bind:value={dialogCostPeriod} />
			</div>
			...
			<div class="flex items-center justify-between rounded-md border border-border bg-muted px-3 py-2">
				...
				<button
					...
					onclick={() => (dialogCostIsEssential = !dialogCostIsEssential)}
				>
					<span class="essential-switch-thumb" class:essential-switch-thumb-on={dialogCostIsEssential}></span>
				</button>
			</div>
			...
			<div class="flex flex-col gap-1.5">
				...
				<Select.Root
					...
					onValueChange={(v: string) => { dialogCostCategoryId = v ?? ''; }}
				>
					...
					<Select.Content class="bg-card border-border">
						{#each categories as cat}
							<Select.Item value={cat.id} class="text-foreground cursor-pointer">
								<span class="flex items-center gap-2">
									<span class="w-2.5 h-2.5 rounded-full flex-shrink-0" style="background: {cat.color ?? DEFAULT_COLOR}"></span>
									{cat.name}
								</span>
							</Select.Item>
						{/each}
		...
		<Dialog.Footer>
			<Button variant="ghost" onclick={() => (costDialogOpen = false)} class="text-muted-foreground">
				Cancel
			</Button>
			<Button
				onclick={costDialogMode === 'add' ? handleAddCost : handleSaveEditCost}
				disabled={submitting || !dialogCostName.trim() || dialogCostAmount <= 0 || !dialogCostCategoryId}
	...
	<Dialog.Content class="bg-card border-border sm:max-w-[500px]">
		...
		<div class="flex flex-col gap-1 max-h-[380px] overflow-y-auto py-1">
			{#each categories as cat (cat.id)}
				{#if editingCategoryId === cat.id}
					<div class="cat-edit-form">
						<div class="flex items-center gap-2">
							<span class="w-3 h-3 rounded-full flex-shrink-0" style="background: {editCatColor}"></span>
							<Input
				...
				{:else}
					<div class="cat-row">
						<span class="w-3 h-3 rounded-full flex-shrink-0" style="background: {cat.color ?? DEFAULT_COLOR}"></span>
		...
		<Dialog.Footer>
			<Button onclick={() => { categoriesDialogOpen = false; editingCategoryId = null; addingCategory = false; }} class="bg-primary text-primary-foreground hover:bg-primary/90">
				Done
			</Button>
		</Dialog.Footer>
	</Dialog.Content>
...
<style>
	...
	.essential-switch {
		...
		transition: background-color 0.16s var(--ds-ease), border-color 0.16s var(--ds-ease);
		padding: 2px;
	}
	...
	.essential-switch-thumb {
		...
		transition: transform 0.16s var(--ds-ease), background-color 0.16s var(--ds-ease);
	}
