import { index, sqliteTable, integer, real, text, uniqueIndex } from 'drizzle-orm/sqlite-core';
...
export const investmentHoldings = sqliteTable(
	'investment_holdings',
	{
		...
		trackerSource: text('tracker_source').$type<'manual' | 'nordea' | 'avanza'>().notNull().default('manual'),
...
export const investmentHoldingSnapshots = sqliteTable(
	...
	(table) => [index('idx_investment_holding_snapshots_holding_id').on(table.holdingId, table.capturedAt)]
);
...
export const wishlistItems = sqliteTable(
	'wishlist_items',
	{
		...
		targetAmountType: text('target_amount_type').$type<'exact' | 'estimate'>().notNull().default('exact'),
		targetDate: text('target_date'),
		priority: integer('priority').notNull().default(5),
		categoryId: text('category_id').references(() => wishlistCategories.id, { onDelete: 'set null' }),
		fundingStrategy: text('funding_strategy').$type<'save' | 'loan' | 'mixed' | 'buy_outright'>().notNull(),
...
export const transactions = sqliteTable(
	'transactions',
	{
		...
		matchMethod: text('match_method').$type<'rule_exact' | 'history_exact' | 'manual' | 'needs_review'>().notNull(),
...
export const merchantCategoryRules = sqliteTable(
	...
	(table) => [
		uniqueIndex('merchant_category_rules_normalized_description_unique').on(table.normalizedDescription),
