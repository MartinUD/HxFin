...
import { getFinancialProfileEffect, updateFinancialProfileEffect } from '$lib/server/finance/service';
import {
...
const budgetHandlers = HttpApiBuilder.group(FinApi, 'budget', (handlers) =>
	handlers
		...
		.handle('updateBudgetCategory', ({ path, payload }) => updateCategoryEffect(path.categoryId, payload))
...
const importsHandlers = HttpApiBuilder.group(FinApi, 'imports', (handlers) =>
	handlers
		...
		.handle('listReviewTransactions', ({ urlParams }) => listReviewTransactionsEffect(urlParams ?? {}))
